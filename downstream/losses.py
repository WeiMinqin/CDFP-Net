...
class FocalLogLoss(nn.Module):
    ...
    def forward(self, y_predict, y_true):
        ...
        loss[y_true != 0] = (
            15
            * torch.pow((right_predictions - 1), self.gamma)
            * torch.log(right_predictions)
        )
...
class LogMaxLoss(nn.Module):
    ...
    def forward(self, y_predict, y_true):
        ...
        loss2 = -5 * torch.sum(
            torch.pow((y_predict - 1), self.gamma) * y_true * torch.log(y_predict),
            dim=1,
        )
